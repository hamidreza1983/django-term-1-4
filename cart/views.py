...
class CheckoutView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        order = Order.objects.create(user=user, phone=phone, address=address, postal_code=postal_code)
        cart = self.request.session.get("cart", {})
        for id, quantity in cart.items():
            service = Services.objects.get(id=int(id))
            quantity = int(quantity)
            OrderItem.objects.create(order=order, service=service, price=service.price, quantity=quantity)
