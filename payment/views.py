    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            if response.get("Status") == 100:
                return redirect(ZP_API_STARTPAY + response.get("Authority"))
            return render(request, 'payment/error.html', context={"error": response.get("Status")})
        except:
            return render(request, 'payment/error.html', context={"error": "خطا در ارتباط با زرین پال"})
